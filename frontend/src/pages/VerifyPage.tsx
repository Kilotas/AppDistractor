...
export default function VerifyPage() {
  ...
  useEffect(() => {
    if (!token) { setStatus("error"); setError(t("tokenMissing")); return; }
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg)" }}>
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, padding: "40px 32px", width: 380, textAlign: "center" }}>
        ...
        {status === "loading" && <p style={{ color: "var(--text-2)" }}>{t("verifying")}</p>}

        {status === "success" && (
          <>
            <div style={{ fontSize: 48, marginBottom: 16 }}>✓</div>
            <p style={{ fontWeight: 600, marginBottom: 8, color: "var(--text)" }}>{t("verifiedTitle")}</p>
            <p style={{ color: "var(--text-2)", marginBottom: 24, fontSize: 14 }}>{t("verifiedDesc")}</p>
        ...
        {status === "error" && (
          <>
            <div style={{ fontSize: 48, marginBottom: 16 }}>✗</div>
            <p style={{ fontWeight: 600, marginBottom: 8, color: "var(--error)" }}>{t("error")}</p>
