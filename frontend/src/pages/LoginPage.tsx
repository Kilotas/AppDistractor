    ...
    return (
      <div style={wrapStyle}>
        <div style={cardStyle}>
          ...
          <h2 style={{ marginBottom: 8, color: "var(--text)" }}>Проверьте почту</h2>
          <p style={{ color: "var(--text-2)", fontSize: 14, marginBottom: 24 }}>
            Мы отправили ссылку для подтверждения на <strong style={{ color: "var(--text)" }}>{email}</strong>
          </p>
          <ResendButton token={null} onResend={async () => { await api.auth.resendVerification(); }} />
      ...
      <div style={cardStyle}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {mode === "register" && (
            <>
              <input
                type="password" placeholder="Подтвердите пароль" value={confirmPassword} required
        ...
        <button onClick={handleGuest} disabled={guestLoading} style={{
          ...
          {guestLoading ? "..." : "Попробовать без регистрации"}
        </button>
      </div>
    </div>
  );
...
function ResendButton({ onResend }: { token: null; onResend: () => Promise<void> }) {
  ...
  if (sent) return <p style={{ fontSize: 13, color: "var(--success)" }}>Письмо отправлено повторно</p>;
  return (
